...
| 3. refresh_runs + drop stored HTML | ⬜ next | | Operational logbook (timings, tokens→real €, per-card facts, errors) separate from customer briefings; HTML rendered on demand instead of stored (10KB data vs ~MB pages). |
| 4. Tunnel Connection Manager + Dockerfile | ⬜ | | Railway-side on-demand `cloudflared access tcp` (port allocation, caps, cleanup); image with ODBC + cloudflared. Hotel-side tunnel stays permanently running. |
| 5. Pipeline hardening | ⬜ | | Per-hotel lock, timeouts soft 2m/warn 3m/hard 8m, publish-after-complete, `degraded` flag, retry ladder 5/15/45min. |
