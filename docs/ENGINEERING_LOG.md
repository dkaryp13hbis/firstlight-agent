...
| _pending_ | 2026-07-29 | Q15 (consumed_by_source): ADR-bridge input — consumed rn+rev by Sourcen, current month (1st→yesterday) vs LY shifted 364 days (weekday-aligned per spec), active bookings only, logis>0 (comps excluded). Fail-open, optional contract field. test_leadtime → 37 checks |
