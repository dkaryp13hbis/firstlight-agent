...
last-year, booking lead time, soft/hot dates (90d), month-end projections, cancellation
spike (z ≥ 2 on Q14, 2026-09-23). Attribution (2026-09-23): the pickup ALERT splits the
swing into cancellations vs slower new bookings (Q14); pace cards name the source that
explains the gap (Q17 `sources_by_month`, only when the per-source deltas reconcile with
the pace gap ±20%) — "concentrated" (≥ 60% one source → source-specific, Medium) vs
"broad" (< 40%, 3+ sources → weaker demand overall, Medium). Score =
...
merge; NOVELTY v2 (2026-09-24): every card carries a fingerprint (`_novelty`: metric / date-set key /
days_out); a card seen in the prior 7 days ships again only when a key token is new, its metric
moved ≥ 10% vs the last sighting, or it entered the last 30 days — no 3-card floor, no scheduled
resurfacing; quiet days are hero-only with a pulse note ("Nothing new in the Pulse today; N items
still open"), payload carries `open_items` + `quiet_day`.
...
**PWA NEXT-YEAR PACE "vs this year" BARS — ✅ SHIPPED 2026-09-13 (PWA `d66b547`, see release history).**

**PARKED — ANALYST CLOSED LOOP (design 2026-09-12, no code, needs a fresh go):**
Anomaly → Investigate → Driver → Decision → Recheck → Outcome. Build order, all Layer A, no extra Claude cost:
- ⬜ **0. Data**: add `source` + `rate_code` and stay-DATE granularity to daily net pickup (Q9) and cancellations (Q14), Protel SQL + Opera `compute_pack`; fail-open (empty list keeps publication).
- ⬜ **1. Driver attribution** for pickup/pace/soft-date cards: decompose the delta into (source × rate plan × new/cancelled) cells; name a driver only at ≥ 60% concentration, else "spread across channels"; direct-channel sanity line ("direct bookings remain normal") when true.
- ⬜ **2. Recheck + outcome line**: every card gets `recheck_date`; when a card stops firing or moves inside the 7-day memory window, emit a deterministic outcome card ("flagged → driver → review → what happened") on real counts; replaces the "Still open" resurface.
- ⬜ **3. Projection ledger**: store the Signal 5 band per hotel-month; on month close record whether the actual fell inside; feeds the outcome line and a trust stat for the Xenia demo.
- ⬜ **4. App**: card body renders the chain as ≤ 4 short lines; resolved cards show a "what happened" strip. Mock from PWA sources first (mock rule).
...
| Date | Decision | Why |
|---|---|---|
| 2026-09-12 | **Product philosophy (design, no code): the analyst is a closed loop — Anomaly → Investigate → Driver → Decision → Recheck → Outcome.** One-liner: "FirstLight searches everything. You see only what matters." (plain form: "FirstLight doesn't give you more data. It does more analysis for you."). Rules that keep the loop defensible: (1) investigation is Layer A, deterministic — Claude narrates the chain, never discovers it; (2) a driver is named only when ONE cell (source × rate plan × new-vs-cancelled) explains ≥ 60% of the delta, otherwise the card says "spread across channels" (itself a finding); (3) the decision stays review/investigate language (action-language rule), never a directive; (4) every card carries a recheck date; outcome is measured on real PMS counts only (room nights, cancellations), never derived €; (5) the outcome line replaces "Still open": flagged → suspected driver → suggested review → what happened; a projection ledger (Signal 5 band vs actual) is the outcome for month-end cards. Known data gap: Q9 daily pickup and Q14 cancellations are grouped by stay MONTH only, with no source and no rate plan; rate plan is fetched nowhere. The example chain needs source + rate code + stay-date granularity on both (Protel SQL + Opera pack), fail-open. PARKED until the user says go | User (2026-09-12): "That, to me, is the product philosophy worth building the entire app around" — combined with the CTO assessment of the same day (driver and outcome are the missing links; attribution needs a concentration gate so the first causal claim is never a false one) |
...
| Commit | Date | What |
|---|---|---|
| (this commit) | 2026-09-24 | NO-REPEAT PULSE (user: "its a bit annoying to be repeating … how do you suggest not repeating the cards" → "ok lets implement that"). Evidence from PG (17–23 Sep, 5 hotels): `hot_dates_near_full` on 29 of 35 hotel-days, the same pace card 6–7 days running, current-month projection daily from the 17th. ROOT CAUSES in the ranking layer: (1) the novelty gate only compared euro stakes, so stake-less cards (hot dates, ahead-pace, projections) were NEVER demoted; (2) the 3-card floor promoted demoted repeats on every quiet day; (3) 'still open' resurfaced every 3rd day by design. FIX: `_fingerprint()` per candidate (pace rn_gap, projection vs_ref, lead_shift, stake; date-set key for hot/soft dates; event-day key for pickup/cancellation/softening; days_out) stored on the insight as `_novelty`; `_novelty_decide()` pure rule (news = new key token, metric moved ≥ 10% vs LAST sighting in either direction, or entered the last 30 days; legacy priors fall back to the stake rule, stake-less legacy repeats are repeats); floor removed; resurfacing removed (the follow-up engine / watchlist is the memory, `watch_closures` already ships the green resolve line); `generate_insights` with 0 ranked cards ships a hero-only briefing (was: legacy prompt call) with `slots["pulse"]` → hero closes with the pulse note (narrated + fallback `_pulse_note_parts`); payload gains `open_items` (demoted repeats: id/tag/title/first_flagged) + `quiet_day`. Also narration rule 15: source codes verbatim (today's Excelsior card expanded "BK" to "Booking.com" — a guess) → `_PROMPT_VERSION cards-v1.10-novelty`. COST: per-card Claude calls, so quiet days drop from ~$0.038 (forced 3 cards) to ~$0.015–0.027 (14-day PG figures: 2 cards $0.027, 3 $0.038, 4 $0.049, 5 $0.059). PWA: AiCards already renders "No insights for today." on empty; rendering `open_items` is a PWA follow-up. Tests: NEW test_novelty.py (38: fingerprints, every decision branch, legacy priors, no-floor via monkeypatched gate, pulse note + hero caps/plain); all other suites unchanged. Day-1 note: tomorrow's priors have no `_novelty` yet → legacy stake rule for one day, full effect from the 26th |
| (this commit) | 2026-09-23 | ANALYST ATTRIBUTION + CANCELLATION SPIKE (user: "yes add 1 and 2" after the FL Pulse review). The cards used to raise questions the snapshot could already answer ("check cancellations by source", "a source-by-source comparison would show which"). (1) **Source attribution on pace cards** — NEW Q17 `Q_SOURCES_BY_MONTH` (OTB by stay month × source, TY vs STLY, Q4 conventions so per-source deltas add up to the pace gap; Protel SQL + Opera Python block; optional/fail-open; contract field `sources_by_month` in `_SIGNAL_PREFIXES` + rows_fetched). `_source_attribution()`: top mover, share of the gap (rounded to 5%), Direct's own move, spread = concentrated (≥ 60%) / broad (< 40%, 3+ sources) / mixed; REFUSES to attribute when the deltas don't reconcile with the pace gap (±20%, min 3 rn) or the top mover is < 5 rn. Behind + concentrated → hypothesis Medium "source-specific issue rather than weaker demand", directive `investigate_source`, fallback evidence row 2 = BIGGEST MOVER; behind + broad → "weaker demand overall" Medium; ahead + concentrated → "{source} is driving most of the lead" as hypothesis #1. (2) **Cancellation attribution on the pickup ALERT** — Q14 `cancel_daily` zero-filled (`_cancel_series`): extra cancels vs the prior-7-day normal ≥ ½ the swing → "Mostly cancellations" (Medium, evidence row 2 = CANCELLED), else "New bookings slowed rather than cancellations rising" (Medium, directive `investigate_demand`). (3) **Signal 6 cancellation spike** — per stay month, z ≥ 2 vs prior 7 days, ≥ 3 rn, stake = real revenue of the cancelled stays ≥ €1,000, finished months skipped, max 2/day, tag ALERT, id `cancel_{mon}_{yyyy}`; `_merge_cancel_into_pickup` folds it into a same-month pickup ALERT (facts prefixed `cancel_`, score = max) so one story never ships twice. No prompt change (narration is generic over the wrapper; `_PROMPT_VERSION` unchanged). Tests: NEW `test_cancel.py` (65: gates, floors, finished month, merge, normal-cancels branch, no-Q14, per-day cap, fallback caps + plain) and `test_source_attr.py` (65: concentrated/broad/non-reconciling/ahead/no-Q17, helper edges); test_contract 17, test_leadtime 37, test_hero 86, test_opera 28, test_followup 24 unchanged. VERIFIED LIVE (build 52dd8bf, 07:00 UTC data-only runs read from Railway PG via the CLI tunnel): `rows_fetched.sources_by_month` = 52 Pome (Protel), 94 City Hotel + 107 ON Residence (Opera), all `success`. Cards with the new facts arrive at the next 03:30 UTC narration — check `cards_audit` for `biggest_source_move` / `cancellations_yday` |
| (this commit) | 2026-09-23 | NOVELTY GATE READS POSTGRES (user: "the database now is postgresql"). `_novelty_gate` still queried Supabase REST directly for the prior 7 days of `briefings`; with STORAGE=pg since 2026-09-11 that worked only because Supabase keeps receiving writes. Now PG-first via `store.get_briefings_since` (same PG-first / Supabase-fallback pattern as railway_main), report_date stringified for psycopg date objects. Behaviour unchanged when PG is off. Tests unchanged (test_store 12) |
| PWA 4ee7404 | 2026-09-23 | SCROLL SPY (user: "when i scroll down to specific section, the navigation doesnt change automatically"). Passive scroll listener + rAF throttle in App: active tab = the LAST `sec-*` anchor at/above the reading line (`fl-sticky` height + 70px); page bottom snaps to the last rendered section (else short final sections are unreachable); `spyMuteUntil` ref mutes the spy for 900ms around tap-driven smooth scrolls so the liquid-glass lens doesn't hop through intermediate sections; spy uses bare `setTab` (no `track('tab_nav')` — taps only). Works in hotel AND portfolio views (portfolio has no sec-overview/sec-ai anchors: top defaults to Overview, bottom stops at Calendar). Verified live (`+900` mute constant in bundle) |
| PWA 1da3d42 | 2026-09-22 | FL Pulse expanded card survived a hotel switch (user: "when i open an fl pulse insight … when i switch hotel the same insight in terms of queue first or 2nd etc is open also"). Card `key` was the bare cardId and card ids repeat across hotels (every hotel ships e.g. `proj_september_2026`), so React reused the instance and its `open` state. Key now `${hotelId}_${cardId}` — fresh collapsed cards on switch, state kept within a hotel. Same bug class as the BarPace tooltip carryover (2026-09-11) and DemandHeat sel reset. Verified live |
| PWA dbfd096 + 0bbf494 | 2026-09-20 | NOTIFICATION DEEP LINK + PER-HOTEL TAGS (user: "when i click to notification for pome it opens the app to the last browsed hotel"). Push payload (`send_typed_push`, all types) now carries `hotelId`; `sw.js` stores it on the notification and on click either postMessages `{NAVIGATE, sectionId, hotelId}` to an open app (App switches via `swRef` — the SW listener is registered once, so current hotels/changeHotel/nav are reached through a ref) or cold-opens `/#sec-ai&h=<id>`, consumed at App module load (writes `fl_hotel` BEFORE the last-browsed restore, then strips the hash). Also fixed: notification `tag` was one shared value, so a multi-hotel user's pushes replaced each other — now `firstlight-<hotelId>`, one notification per hotel. Per-hotel notification CHOICE already existed (bell + morning/alerts/momentum prefs are per (user, hotel) row) — answered, nothing built. Verified live: sw.js carries hotelId, bundle carries the `[#&]h=` consumer, backend /health 0bbf494 |
| PWA 6045021 | 2026-09-20 | Hotel pace charts trimmed 15% (user: "reduce a bit the height around 15%"). New `mid` height in `BarPace`/`OccPace` (plot 223px vs tall's 262; same 30px top margin + 68px under-axis) used only by `OtbTab`; portfolio keeps `tall` untouched. Verified live (`=223` constant in bundle) |
| PWA 4769c47 | 2026-09-20 | Hotel pace/OTB charts get the portfolio layout (user: "adjust the OTB charts to be bigger and fit better in the shape for the standalone hotels, [like the] multiproperty layout charts"). `OtbTab`'s three charts (Revenue OTB / Occupancy / ADR) now render `tall` (plot 262px vs 172px, ≈1.5× taller) with the −10px width bleed into the card padding, both lifted from `Portfolio.tsx`. `BarPace` bar width decoupled from the `tall` flag → follows the month step (`clamp(10, step·0.22, 15)`): 12-month hotels/portfolio keep 10px bars (portfolio pixel-identical), a 7-month seasonal hotel keeps full 15px bars in the taller plot. Applies to this-year and next-year views. Verified live (bleed literal ×4 in bundle) |
| PWA 4e73d9f | 2026-09-16 | Demo account gets multiproperty (user: "yes add demo for the multiproperty fictional data"). demo@hbis.io added to `PORTFOLIO_PREVIEW_EMAILS` → the fictional 8-hotel "Portfolio · preview" now appears in the demo login's hotel picker. Verified live (bundle carries the demo email once). The REAL portfolio stays unavailable to demo by design: Azure Bay / Thalassa Palace are `active=false` (keeps the scheduler off them) and `portfolio_groups` requires ≥2 ACTIVE hotels in an active group. A matching real demo group (portfolio built from the seeded demo briefings) is parked as a possible pre-Xenia polish item |
| (this commit) | 2026-09-14 | **INCIDENT: POME + CITY HOTEL BRIEFINGS BLOCKED ALL DAY (user: "can you check why we have errors on pome").** Every 2026-09-14 full run for Pomegranate and City Hotel failed `TypeError: Object of type Decimal is not JSON serializable` (7–8 attempts each, 03:32→07:08 UTC), and all 5 intraday data-only runs then failed `no_morning_ai` — neither hotel published a briefing on 09-14. Potidea / Excelsior / ON Residence unaffected. ROOT CAUSE: `watchlist.first_gap`/`last_gap` are `numeric` in PG, and `store._norm()` normalized uuid/date/datetime but NOT `Decimal` — `followup.update_followups()` copied the row's `first_gap` verbatim into the card's `follow_up` meta, and `cloud_push` (`requests.post(json=payload)` → stdlib `json.dumps`) crashed on it. Data-dependent trigger, which is why it slept for 3 days: the meta only attaches when a matching month card ships on day ≥ 2 of a FirstLight watch — exactly the two hotels with open watches got a month card again on 09-14 (Pome October flagged 09-11 at −53.6%). The 09-13 evening deploy (ed548e5) was innocent. FIX: `_norm` converts `Decimal → float` (root fix, covers every PG read) + `update_followups` coerces both gaps to plain floats before publishing (defence in depth). `test_followup.py` regression: Decimal-typed rows through `update_followups` must yield float gaps and a json-serializable `ai` (24 checks; test_store 12 unchanged). Recovery: manual full runs re-queued for both hotels after deploy — narration is allowed because no `ai_insights` exists for the day |
| (pending push) + React | 2026-09-12 | **MULTI-PROPERTY PORTFOLIO — REAL DATA (user: "lets show also the multiproperty").** Backend `briefing/portfolio.py`: builds the app's `PortfolioData` from every member hotel's LATEST stored briefing — NO new PMS fetch: yd/mtd from the blocks, **YTD = past months of `pace` (TY actuals + Final LY) + MTD**, 12 months from `pace`, pickup 1/3/7 from the card + **14-day from `pickup_daily` + `cancel_daily`**, 30-day calendar from `otb_by_date`, `open` months from `season_settings` (null = all year), stale = report_date older than the group's newest, `netFactor` = Σnet/Σgross of yesterday (replaces the fixture's ÷1.13). `store.portfolio_groups` (groups with ≥2 of the caller's active hotels) + `store.portfolio_group_hotels` (membership-filtered). `GET /app/portfolios`, `GET /app/portfolio?group_id=` (user JWT + hotel_users). `test_portfolio.py` 22 checks; verified against the live Tor briefings (YTD City 23,402 rn / 2,819,075 € = the validation harness). **App** (`firstlight-pwa`): picker gets one entry per group (`portfolio:<id>`, after the hotels, admin fixture preview kept last); `PortfolioView` takes `data`/`state` (loading / unavailable cards), dynamic MTD/YTD/pickup-window labels from the report date, real footer text, stale banner explains which hotel; net uses `netFactor`. `tsc -b` + `vite build` clean. Membership rule: an owner sees the whole group, a viewer on one hotel sees only that row |
...
| (this commit) | 2026-09-13 | FL PULSE HEADLINE CUT WITH "…" (user: "ON Residence FL Pulse second insight stops with 3 dots"). Stored card `proj_september_2026` (2026-09-12 briefing, run d86869bc, `fallback_used: true` after the Claude draft failed `by_when is 12 words (max 10)`) shipped headline "September on track to finish around +8% to +13% vs final last…" — the app renders headlines whole, the cut is in the data. ROOT CAUSE: the current-month MONITOR projection fallback template was 13 words with a 3-word band + "final last year" (headline cap 12), so `_enforce_caps()` — the last-resort clamp — trimmed it and appended "…". The word-limit contract says fallback templates are test-enforced within caps; the projection templates were not covered. FIX: template → "{month} on track for {band} vs {ref}" (11 words); `test_hero.py` now runs `_compute_signals` through all four projection branches (current month ahead/behind, future month ahead/behind) and checks every fallback field is within `_WORD_CAPS` and never ends with "…" (86 checks pass; test_leadtime 37, test_contract 17, test_opera 28 unchanged). Still at exactly 12 words, passing: the "expected to finish around …" headlines (current-month behind, future-month ahead) — no headroom, left as is. Today's stored ON Residence card stays cut until the next 03:30 UTC narration (manual runs reuse the day's ai_insights) |
| PWA `d66b547` | 2026-09-13 | NEXT-YEAR PACE: STLY IS ALWAYS THE SAME-STAGE VALUE (user: "Opera SQL, 2027 vs 2026 same time last year looks very big" → "OTB chart doesn't show the final LY but STLY as 275470"). VERIFIED FIRST from City Hotel's stored 2026-09-12 briefing (Railway PG via `railway connect Postgres --tunnel-only`): adapter Jan = 2027 OTB 85 rn / €13,230 · 2026 at the same stage 156 rn / €15,386 · 2025 at the same stage 128 rn / €14,192 · Jan 2026 final 2,611 rn / €275,470 — the Opera STLY series is sane (reconciled to Hotel BI 2026-09-11, adapter untouched). ROOT CAUSE in the app: `Charts.tsx buildNextPace(comp='this')` put this year's ACTUAL in the STLY slot for closed months (Jan–Aug) and drew no Final LY bar, heading still said "same stage"; invisible on Pome (closed-season viewing reads as "last year's final"), glaring on a year-round city hotel. FIX: STLY = `rn_stly`/`rev_stly` on all 12 months; closed months of this year expose their actual as the FINAL reference (green dashed + tooltip), open months have none; new `open` prop on BarPace/OccPace for the next-year view (no month is "closed" by calendar there — Jan 2027 is open — so pills/tooltips compare to STLY everywhere; axis scales to OTB vs STLY only, a full-month final would squash the next-year bars, final line drawn only when it fits, tooltip always has it; this also un-squashes the "vs last completed year" view). Settings + reporting-year strip wording updated. `npm run build` clean; `npm run lint` fails on an oxlint native-binding load error (environment, pre-existing). Backend: no change |
...
- ✅ 2026-09-12 **Hotel picker opens in place** (React a582597,
  live q8F3xl6D; user: tapping the dropdown while scrolled down "just
  jumps up"): the picker panel moved OUT of the header's collapsing
  row 2 and anchors to the header itself — opens wherever you are, in
  both header states; portal-backdrop closes it on outside tap; the
  old scroll-to-top-then-open shortcut removed. Hotel switch already
  resets to Overview + top (§8), unchanged.

- ✅ 2026-09-12 **Since Yesterday paint fixes** (React dc16996, live
  4FFF_Se4; user: section shows empty then fills / "shows the hero card
  and then suddenly appears under it"): the section needs YESTERDAY'S
  briefing, which was fetched (not cached) → late mount + layout shift.
  Fix: fl_prev_<hotel> cache (written on fetch, cleared on sign-out) →
  first-frame paint on every repeat open; when no cache exists yet, a
  same-height fl-skel placeholder holds the slot so the layout never
  shifts. Content semantics unchanged: recomputes on every data load —
  latest intraday state vs yesterday's briefing (drifts through the
  day by design).
