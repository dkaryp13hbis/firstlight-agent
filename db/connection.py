import pyodbc
import config


def connect_mssql(host: str, port: int, user: str, password: str,
                  database: str = "bidata",
                  driver: str | None = None,
                  timeout: int = 30) -> pyodbc.Connection:
    """Connect to a SQL Server by explicit address — used by the Railway
    tunnel-direct path (host is a local cloudflared port). Encryption is
    disabled: on-prem Protel/Pylon SQL Servers typically lack TLS certs, and
    the Cloudflare tunnel already encrypts the wire.

    Driver order: prefer ODBC 17 (tolerates the ancient TLS some on-prem
    SQL Servers speak in the TDS prelogin), fall back to 18. A newer
    msodbcsql18 broke Pome's prelogin handshake (incident 2026-08-10) while
    the same container spoke to Potidea's newer SQL Server fine."""
    installed = set(pyodbc.drivers())
    candidates = ([driver] if driver else
                  [d for d in ("ODBC Driver 17 for SQL Server",
                               "ODBC Driver 18 for SQL Server") if d in installed]
                  or ["ODBC Driver 18 for SQL Server"])
    last_exc: Exception | None = None
    for drv in candidates:
        conn_str = (
            f"DRIVER={{{drv}}};"
            f"SERVER={host},{port};"
            f"DATABASE={database};"
            f"UID={user};"
            f"PWD={password};"
            f"Encrypt=no;TrustServerCertificate=yes;"
        )
        try:
            return pyodbc.connect(conn_str, timeout=timeout)
        except pyodbc.Error as exc:
            last_exc = exc
    raise last_exc


def get_connection() -> pyodbc.Connection:
    if config.SQL_TRUSTED.lower() == "yes":
        conn_str = (
            f"DRIVER={{{config.SQL_DRIVER}}};"
            f"SERVER={config.SQL_SERVER};"
            f"DATABASE={config.SQL_DATABASE};"
            f"Trusted_Connection=yes;"
        )
    else:
        conn_str = (
            f"DRIVER={{{config.SQL_DRIVER}}};"
            f"SERVER={config.SQL_SERVER};"
            f"DATABASE={config.SQL_DATABASE};"
            f"UID={config.SQL_USER};"
            f"PWD={config.SQL_PASSWORD};"
        )
    return pyodbc.connect(conn_str, timeout=30)
