"""
Shared HotelDataSnapshot assembly — one implementation for every PMS adapter.

An adapter's job is to produce the *query result rows* (dicts with the column
names of the Protel query pack, see `db/adapters/protel_mssql/queries.py`);
this module turns those rows into the contract payload (`db/contract.py`).
Protel gets the rows from 16 SQL queries; Opera computes the same rows in
Python from one bounded extract. Either way the payload — and therefore the
analyst, cards and app — is identical.

`assemble_snapshot(rows, hotel_ctx)` expects a dict with these keys (missing
optional keys → empty, exactly like a fail-open query):

    kpi           one dict  (Q1  Q_KPIS)
    inventory     list      (Q7  Q_INVENTORY:  ref_date, total_rooms)
    inhouse       one dict  (Q2  Q_INHOUSE)
    pickup        one dict  (Q3  Q_PICKUP)
    pace          list      (Q4  Q_PACE)
    sources       list      (Q5  Q_SOURCES_OTB)
    next7         list      (Q6  Q_NEXT7)
    curve         list      (Q8  Q_BOOKING_CURVE)
    curve_full    list      (Q8b Q_BOOKING_CURVE_FULL_MONTHS)
    pickup_daily  list      (Q9  Q_PICKUP_DAILY)
    otb_by_date   list      (Q10 Q_OTB_BY_DATE_90)
    current_month one dict  (Q11 Q_CURRENT_MONTH_REMAINING)
    lead_time     list      (Q13 Q_LEAD_TIME)          optional
...
def assemble_snapshot(rows: dict[str, Any], hotel_ctx: dict[str, Any],
    ...
    rev_yd_ty  = float(kpi.get("rev_yday_ty") or 0)
    rev_yd_ly  = float(kpi.get("rev_yday_ly") or 0)
    rn_yd_ty   = float(kpi.get("rn_yday_ty")  or 0)
    rn_yd_ly   = float(kpi.get("rn_yday_ly")  or 0)
    rev_mtd_ty = float(kpi.get("rev_mtd_ty")  or 0)
    ...
    rev_mtd_ly = float(kpi.get("rev_mtd_ly")  or 0)
    rn_mtd_ty  = float(kpi.get("rn_mtd_ty")   or 0)
    rn_mtd_ly  = float(kpi.get("rn_mtd_ly")   or 0)
    ...
    # ── Q2: In-house ──────────────────────────────────────────────
    ih = rows.get("inhouse") or {}
    arrivals   = ih.get("arrivals")   or 0
    departures = ih.get("departures") or 0
    stayovers  = ih.get("stayovers")  or 0

    # ── Q3: Pickup ────────────────────────────────────────────────
    pu = rows.get("pickup") or {}
    pickup_today_rn  = float(pu.get("pickup_today_rn")  or 0)
    pickup_today_rev = float(pu.get("pickup_today_rev") or 0)
    pickup_1d_rn     = float(pu.get("pickup_1d_rn")     or 0)
    pickup_1d_rev    = float(pu.get("pickup_1d_rev")    or 0)
    pickup_3d_rn     = float(pu.get("pickup_3d_rn")     or 0)
    pickup_3d_rev    = float(pu.get("pickup_3d_rev")    or 0)
    pickup_7d_rn     = float(pu.get("pickup_7d_rn")     or 0)
    pickup_7d_rev    = float(pu.get("pickup_7d_rev")    or 0)
    cancel_today_count = float(pu.get("cancel_today_count") or 0)
    cancel_today_rev   = float(pu.get("cancel_today_rev")   or 0)
    cancel_1d_count  = float(pu.get("cancel_1d_count")  or 0)
    cancel_1d_rev    = float(pu.get("cancel_1d_rev")    or 0)
    cancel_3d_count  = float(pu.get("cancel_3d_count")  or 0)
    cancel_3d_rev    = float(pu.get("cancel_3d_rev")     or 0)
    cancel_7d_count  = float(pu.get("cancel_7d_count")  or 0)
    cancel_7d_rev    = float(pu.get("cancel_7d_rev")     or 0)
    top_month_num    = int(pu.get("top_month")           or today.month)
    top_month_rn     = float(pu.get("top_month_rn")      or 0)

    # ── Q4: Pace ──────────────────────────────────────────────────
    pace = []
    for r in rows.get("pace") or []:
        m = int(r["stay_month"])
    ...
    curve_full = _build_curve_months_by_bookmonth(rows.get("curve_full") or [], stly_cap.month)
    curve_full["month_label"] = f"Full Year {today.year}"

    # ── Q9: Daily pickup for last 14 days by future stay month ───
    pickup_daily = []
    for r in rows.get("pickup_daily") or []:
        ref = r["ref_date"]
    ...
    otb_by_date = []
    for r in rows.get("otb_by_date") or []:
        sd = r["stay_date"]
    ...
    lead_time = []
    for r in rows.get("lead_time") or []:
        lead_time.append({
            "period":      r["period"],
            "stay_month":  int(r["stay_month"]),
            "stay_year":   int(r["stay_year"]),
    ...
    # ── Q14: Daily cancellations by stay month ────────────────────
    cancel_daily = []
    for r in rows.get("cancel_daily") or []:
        ref = r["ref_date"]
        if hasattr(ref, "strftime"):
            ref = ref.strftime("%Y-%m-%d")
        cancel_daily.append({
    ...
    # ── Q15: Consumed rn+rev by source, MTD vs LY-364d ────────────
    consumed_by_source = []
    for r in rows.get("consumed") or []:
    ...
    dq = attach_data_quality(payload, total_rooms)
    if not dq["complete"]:
        print(f"[{hotel_name}] DATA QUALITY WARNING — snapshot incomplete: "
